...
function App() {
  ...
  const loadMcEditOptions = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/mc/fake-values`)
      ...
      setMcEditRows((prev) => {
        if (!prev.length) {
          return [{ id: 1, device: '', address: '', value: '' }]
        }
        return prev.map((row) => {
          const byAddress = row.address ? entries.find((e) => e.address === row.address) : null
          const byName = !byAddress && row.name ? entries.find((e) => e.name === row.name) : null
          const option = byAddress || byName
  ...
  return (
    <div className="app">
      ...
      <McEditModal
        ...
        onClose={closeMcEditPopup}
        onAddRow={handleAddMcEditRow}
        onDeviceChange={handleMcEditDeviceChange}
        onAddressChange={handleMcEditAddressChange}
        onValueChange={handleMcEditValueChange}
        onRemoveRow={handleRemoveMcEditRow}
        onSave={handleMcEditSave}
        onReload={loadMcEditOptions}
      />
